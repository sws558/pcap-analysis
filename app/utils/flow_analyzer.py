...
from scapy.all import (send, corrupt_bytes, Raw, TCP, IP, RandIP,
                       RandShort, RandByte, conf, IPv6, UDP, ARP, ICMP, DNS, ICMPv6ND_NS)
import collections
import time
...
def get_flow_ip(PCAPS):
    flow_ip = list()
    for pcap in PCAPS:
        if pcap.haslayer(IP):
            flow_ip.append({"src_ip": pcap.getlayer(IP).src, "tst_ip": pcap.getlyer(IP).dst})
