...
def get_time_pcap(pcap, start_time, end_time, PD):
    # 筛选时间,vivews，line 184
    x=dict()
    pcaps = collections.OrderedDict()
    count = 1
    ...
    for p in pcap:
        x = pcap[p]
        if "time" in x:
            pcap_time_str = x['time']#字符串？
            pcap_time = datetime.strptime(pcap_time_str, "%Y-%m-%d %H:%M:%S")  
            if start_time <= pcap_time and pcap_time<= end_time:
                pcaps[count] = x
                count += 1
    return pcaps
