    ...
    for web in webdata:
        ...
        for raw_data, data in zip(raw_data_list, data_list):
            if start:
                ...
                file_name = (
                    type + "_" + web["ip_port"].split(":")[0] + "_" + web["ip_port"].split(":")[1] + "_" + filename)
                fn = os.path.join(folder, file_name)
                with open(fn, "wb") as f:
                    f.write(raw_data.strip())
                web_list.append({
                    "ip_port": web["ip_port"].split(":")[0] + ":" + web["ip_port"].split(":")[1], "filename": (fn), "size": "%.2f" % (os.path.getsize(fn) / 1024.0), }
                )
                start = False
                switch = False
            if switch:
            ...
            if "GET" in data and "HTTP/1.1" in data:
                try:
                    ...
                    if re.match(r"^[A-Za-z0-9_]*?\.[A-Za-z0-9_]*?$", filename):
                        filename = re.match(
                            r"^[A-Za-z0-9_]*?\.[A-Za-z0-9_]*?$", filename
                        ).group()
                        switch = True
                        start = False
                        type = "GET"
        ...
        if "PASV" in cmd_data:  # PASV模式,通过Web浏览器访问模式
            ...
            for ftp in ftpdata[start:]:
                ...
                ftp_list.append({"ip_port": ftp["ip_port"].split(":")[0] + ":" + ftp["ip_port"].split(":")[1],
                                "filename": folder + file_name, "size": "%.2f" % (os.path.getsize(folder + file_name) / 1024.0), })
...
def mail_file(PCAPS, host_ip, folder):
    ...
    for mail in maildata:
        ...
        for data in data_list:
            if switch:
                ...
                try:
                    ...
                    pass
                file_dict[mail["ip_port"].split(":")[0] + "_" + mail["ip_port"].split(":")[1] + "_" + filename_p] = filedata
                switch = False
            if "filename" in data:
                switch = True
        ...
        for filename, filedata in file_dict.items():
            ...
            mail_list.append(
                {
                    "ip_port": filename.split("_")[0] + ":" + filename.split("_")[1],
