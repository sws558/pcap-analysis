...
def web_data(PCAPS, host_ip):
    ...
    for pcap in PCAPS:
        if pcap.haslayer(TCP) and pcap.haslayer(Raw):
            ...
            elif pcap.getlayer(IPv6):
                src = pcap.getlayer(IPv6).src
                dst = pcap.getlayer(IPv6).dst
            else:
                src = "unknow"
                dst = "unknow"
            sport = pcap.sport
    ...
    for data in data_list:
        if switch:
            ...
            try:
                ...
                pass
            attachs_dict[filename_p] = filedata
            switch = False
        if "filename" in data:
            switch = True
...
def client_info(PCAPS):
    ...
    for sess, ps in sessions.items():
        ...
        for p in ps:
            if p.haslayer(Raw):
                # payload += p[Raw].load
                payload_list.append(p[Raw].load.decode("UTF-8", "ignore"))
            # if payload:
    ...
    for sess, payload in allpayloads_dict.items():
        ...
        for pattern in client_patterns:
            ...
            if client:
                clients_str = ";".join([client[0], clients_str])
